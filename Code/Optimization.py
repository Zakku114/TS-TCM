...
# 函数
from Modules import *
from utils import get_geneid_symbol_mapping,get_union,get_intersection,get_adj,get_avg_length,get_link,read_recipes,read_drugs,\
...
if not linked:
    # 读取靶点:社区下标的映射
    with open('../Output/cluster_results.txt', 'rb') as handle:
    ...
    # 读取节点的邻接矩阵
    with open('../Output/BeforeH.txt', 'rb') as handle:
...
if not dataset_generated:
    # 读取节点的相似度矩阵
    with open('../Dataset/similarity.txt','r') as f:
        lines = f.readlines()
        similarity = []
        for line in lines:
            line = line.strip("\n").split(' ')
    ...
    # 存储over_H和H
    with open('../Output/OverH.txt', 'wb') as handle:
        pickle.dump(over_H, handle, protocol=pickle.HIGHEST_PROTOCOL)
    handle.close()
    with open('../Output/H.txt', 'wb') as handle:
        pickle.dump(H, handle, protocol=pickle.HIGHEST_PROTOCOL)
    handle.close()
else:
    # 读取over_H和H
    with open('../Output/H.txt', 'rb') as handle:
        H = pickle.load(handle)
    handle.close()
    with open('../Output/OverH.txt', 'rb') as handle:
...
if not trained:
    ...
    optimizer = torch.optim.Adam(params_list, lr=1e-4)
    # 训练
...
if not hyper_generated:
    ...
    with open('../Output/afterH.txt', 'wb') as handle:
...
# 读取中药组方
recipes_dir =  '../Dataset/all_recipes.xlsx'
recipes_id = read_recipes(recipes_dir,nodes)
   
# 读取背景中药数据集
drugs_dir = '../Dataset/all_drugs.xlsx'
drugs_id = read_drugs(drugs_dir,nodes)
    ...
    for count in range(epoch):
        ...
        with torch.no_grad():
            ...
            if(score>best_score):
                ...
                f.close()
                # 获取用于检验结果的数据集
                with open('test_recipes.txt','wb') as f:
                    pickle.dump(Test_x,f)
                f.close()
                with open('test_label.txt','wb') as f:
...
for i in range(1000):
    ...
    # random 
    temp = set()
    drug_number = random.randint(4,5)
...
with torch.no_grad():
    ...
    for i in range(len(pred_y)):
        #if pred_y[i]>0.5 and pred_y[i]<0.99:
        if pred_y[i]>1e-4:
            score.append(pred_y[i])
            screened.append(res[i])
print(sum(score)/len(score))
exit()
f = open('../Output/screening.txt', 'w')
