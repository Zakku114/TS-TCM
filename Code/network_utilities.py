...
try:
    from external import NWSteiner, Heuristic, kWalk
except:
...
def get_heuristic_tree(G, terminals, distance=2, score_increment=0.2):
    ...
    subgraph = Heuristic.listQuery(G, terminals, d=distance,
                                   r=score_increment)  # d=1, r=0.2, scoreFun=Heuristic.sum_score
    return subgraph
...
def get_edge_betweenness_within_subset(G, subset, edges, consider_alternative_paths=False):
    ...
    if consider_alternative_paths:
        for i, u in enumerate(subset):
            # enumerate() 函数用来遍历一个集合对象，它在遍历的同时还可以得到当前元素的索引位置
...
def get_background_normalized_separation_distance(network, sp, targets, seeds, distance="shortest", n_random=100,
                                                  n_node_in_bins=100,
                                                  seed_to_equivalent_nodes=None, seed=None):
    if seed is not None:
        random.seed(seed)
...
def get_source_to_average_target_distance_random(sp, targets, nodes, distance, target_mean_and_std,
                                                 seed_to_equivalent_nodes,
                                                 n_random=100):
    target_to_values = {}
    ...
    for i in range(n_random):
        ...
        for seed, equivalents in seed_to_equivalent_nodes.iteritems():
            nodes.append(random.choice(equivalents))
        target_to_distance_random = get_source_to_average_target_distance(sp, targets, nodes, distance=distance,
                                                                          target_mean_and_std=target_mean_and_std)
...
def get_source_to_average_target_distance(sp, geneids_source, geneids_target, distance="shortest", parameters={},
                                          target_mean_and_std=None, exclude_self=False):
    """
    Returns average/min distance to target nodes for each source node
    ...
    if distance.startswith("mahalanobis") or distance == "center":
        ...
        target_to_distance = get_source_to_average_target_distance(sp, geneids_target, geneids_target,
                                                                   distance="closest", parameters=parameters,
                                                                   target_mean_and_std=None, exclude_self=True)
    ...
    if "subset" in parameters:
        geneids_source_mod, geneids_target_mod = get_optimal_subsets_using_parameters(parameters, sp, geneids_source,
                                                                                      geneids_target)
...
def get_optimal_subsets_using_parameters(parameters, sp, geneids_source, geneids_target):
    ...
    if optimal_subset == "seeds":  # subseting seeds (geneids_target)
        geneids_target_sub = get_optimal_subset(sp, geneids_source, geneids_target, n_cutoff, d_cutoff, k_cutoff,
                                                network)
    elif optimal_subset == "targets":  # subsetting targets (geneids_source)
        geneids_source_sub = get_optimal_subset(sp, geneids_target, geneids_source, n_cutoff, d_cutoff, k_cutoff,
                                                network)
        ...
        geneids_target_sub = get_optimal_subset(sp, geneids_source, geneids_target, n_cutoff, d_cutoff, k_cutoff,
                                                network)
        # subset targets
        geneids_source_sub = get_optimal_subset(sp, geneids_target, geneids_source, m_cutoff, d_cutoff, k_cutoff,
                                                network)
    else:
...
def get_optimal_subset(sp, geneids_source, geneids_target, n_cutoff=FINITE_INFINITY, d_cutoff=FINITE_INFINITY,
                       k_cutoff=0, network=None):
    """
...
def get_separation(network, sp, targets, seeds, distance, parameters={}, averaging_function=lambda x: numpy.mean(x)):
    ...
    if distance.startswith("jorg-"):
        ...
        target_to_distance = get_source_to_average_target_distance(sp, seeds, seeds, distance=distance,
                                                                   parameters=parameters, target_mean_and_std=None,
                                                                   exclude_self=True)
        values = target_to_distance.values()
        d1 = averaging_function(values)
        target_to_distance = get_source_to_average_target_distance(sp, targets, targets, distance=distance,
                                                                   parameters=parameters, target_mean_and_std=None,
                                                                   exclude_self=True)
        values = target_to_distance.values()
        d2 = averaging_function(values)
        target_to_distance = get_source_to_average_target_distance(sp, targets, seeds, distance=distance,
                                                                   parameters=parameters, target_mean_and_std=None)
        values = target_to_distance.values()
        target_to_distance = get_source_to_average_target_distance(sp, seeds, targets, distance=distance,
                                                                   parameters=parameters, target_mean_and_std=None)
        ...
        target_to_distance, center_d = get_source_to_average_target_distance(sp, targets, seeds,
                                                                             distance="mahalanobis-shortest",
                                                                             parameters=parameters)
        values = target_to_distance.values()
        target_to_distance, center_d = get_source_to_average_target_distance(sp, seeds, targets,
                                                                             distance="mahalanobis-shortest",
                                                                             parameters=parameters)
        values.extend(target_to_distance.values())
        ...
        if distance.endswith("tom") or distance.endswith("tom-min"):
            ...
            target_to_distance = get_source_to_average_target_distance(sp, seeds, targets, "closest",
                                                                       parameters=parameters)
        else:
...
def convert_distance_matrix_to_dictionary(D, n, idx_to_node):
    sp = {}
    for i in range(0, n):
        u = idx_to_node[i]
        sp[u] = {}
...
def get_degree_binning(g, bin_size, lengths=None):
    ...
    # 对度数排序，升序
    values = list(sorted(degree_to_nodes.keys()))
    # 对节点按度数分层
    bins = []
    i = 0
    while i < len(values):
        ...
        count = 0
        dataset = []
        while count < bin_size-1 and i < len(values):
            count += len(degree_to_nodes[values[i]])
            dataset.extend(degree_to_nodes[values[i]])
            i += 1
        high = values[i-1]
        bins.append((low,high,dataset))
    return bins
...
def get_degree_equivalents(seeds, bins, g):
    ...
    for seed in seeds:
        ...
        for l, h, nodes in bins:
            if l <= d and h >= d:
                mod_nodes = list(copy.deepcopy(nodes))
    ...
    for i in range(n_random):
        if degree_aware:
            ...
            for node, equivalent_nodes in node_to_equivalent_nodes.items():
                ...
                for k in range(10):  # Try to find a distinct node (at most 10 times)
...
def output_network_in_sif(g, output_file_name, node_to_desc=None, delim=" ", include_unconnected=True,
                          remove_self=True):
...
def analyze_network(g, out_file=None, seeds=None, calculate_radius=False):
    ...
    if calculate_radius:
        out_method("Radius (of the largest connected component): %d\n" % get_network_radius(
            g.subgraph(connected_components[0])))
...
def randomize_graph(graph, randomization_type, allow_self_edges=False):
    ...
    if randomization_type == "random":
        ...
        new_graph.add_edges_from(
            [(equivalences[current_edge[0]], equivalences[current_edge[1]], graph.get_edge_data(current_edge[0],
                                                                                                current_edge[1])) for
             current_edge in graph.edges()])
...
def create_dot_network_file(g, output_file, seeds=set(), node_to_desc=dict(), ups=set(), downs=set(), weak_edges=set(),
                            draw_type="all"):
    """
    roots seeds
    ...
    if draw_type == "linker annotated":
        ...
        for node in g.nodes():
            if node in seeds:
                f.write(
                    "%s [label=\"%s\" style=filled fillcolor=red root=true color=red height=0.05 width=0.05 shape=rect];\n" % (
                        node, node_to_desc[node]))
            elif node in linkers:
                if node in ups:
                    f.write(
                        "%s [label=\"\" style=filled fillcolor=green color=green fixedsize=true height=0.05 width=0.05 shape=rect];\n" % (
                            node))
                elif node in downs:
                    f.write(
                        "%s [label=\"\" style=filled fillcolor=blue color=blue fixedsize=true height=0.05 width=0.05 shape=rect];\n" % (
                            node))
                else:
                    f.write("%s [label=\"\" fixedsize=true height=0.05 width=0.05 shape=rect];\n" % (node))
        ...
        for node in g.nodes():
            if node in seeds:
                ...
                f.write(
                    "%s [label=\"%s\" style=filled fillcolor=green color=green fixedsize=true height=0.05 width=0.05 shape=rect];\n" % (
                        node, node_to_desc[node]))
            elif node in downs:
                f.write(
                    "%s [label=\"%s\" style=filled fillcolor=blue color=blue fixedsize=true height=0.05 width=0.05 shape=rect];\n" % (
                        node, node_to_desc[node]))
