...
from utils import get_geneid_symbol_mapping,get_nodes,get_union,get_intersection,get_enrichment,save_key_nodes,save_sub_network,\
                  load_dataset,feature_generator,to_sparse_tensor,adj_polynomials,cluster_infer,community_to_node
...
# 靶点按照度数切片的参数
parser.add_argument('--bin_num', type=int, default=50, help='node split by degree')
# 特征生成方式，目前仅支持svd降维
parser.add_argument('--preprocess', type=str, default='svd', help='feature preprocessing: svd')
# svd降为多少维
parser.add_argument('--n_components', type=int, default=240, help='n_components for SVD')
# 邻接矩阵最大阶数
...
parser.add_argument('--batch_size', type=int, default=1000, help='batchsize')
# 模型&结果存储路径
...
# 社区数量
parser.add_argument('--K', type=int, default=250, help='cluster_number')
# 是否对输入数据进行串标准化
...
# 组方/疾病靶点文件
sheetname = '../Dataset/all_recipes.xlsx'
# 保存富集功能的文件
function_file = "../Dataset/functions.txt"
# 基因本体论相关数据集
...
# 保存相似度矩阵和相互作用矩阵
similarity_file = '../Dataset/similarity.txt'
interaction_file = '../Dataset/interaction.txt'
...
# 读取该疾病涉及的全部靶点
names,nodes = get_nodes(sheetname,geneid_to_name,name_to_geneid)
...
if not CommunitySlash:
    ...
    # 记录模型及模型中间结果
    model_out = args.outputdir +"CommunityModel" + ".pth"
    ...
    # 结果保存
    with open(args.outputdir + 'cluster_results.txt', 'wb') as handle:
        pickle.dump(clust_results, handle, protocol=pickle.HIGHEST_PROTOCOL)
    handle.close()
    with open(args.outputdir + 'community_results.txt', 'wb') as handle:
